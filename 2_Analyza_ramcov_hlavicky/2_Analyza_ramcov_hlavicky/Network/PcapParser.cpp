...
vector<Frame *> PcapParser::parse(const string &fileName) {
	...
	while ((readingResult = pcap_next_ex(file, &parsingHeader, &parsingData)) == 1) {
...
Frame *PcapParser::parseFrame() {
	if (parseFrameLength() > 1500)
		return new EthernetIIFrame(serialNumber, parsingHeader->len, (uint8_t *)parsingData, parsingHeader->caplen);
	else
		return parseIeee802_3Frame();
}

uint16_t PcapParser::parseFrameLength() {
	return ntohs(*(uint16_t *)(((uint8_t *)parsingData) + 12));
}

Frame *PcapParser::parseIeee802_3Frame() {
	switch (parseDSAP()) {
		case 0xFF:
			return new Ieee802_3RawFrame(serialNumber, parsingHeader->len, (uint8_t *)parsingData, parsingHeader->caplen);
			
		case 0xAA:
			return new Ieee802_3LlcSnapFrame(serialNumber, parsingHeader->len, (uint8_t *)parsingData, parsingHeader->caplen);
			
		default:
			return new Ieee802_3LlcFrame(serialNumber, parsingHeader->len, (uint8_t *)parsingData, parsingHeader->caplen);
	}
}

uint8_t PcapParser::parseDSAP() {
	return *(((uint8_t *)parsingData) + 13);
}
